...
void timer(int val1)
{
    ...
    theta2 += 0.1;
    if(theta2>2.0*M_PI) theta2 -= 2.0*M_PI;
        
        
        
        //----------ここに毎フレーム実施する内容を記述----------
        theta1 = theta*2;
        if (theta1>60)theta1 = 60;
        else if (theta1<-60) theta1 = -60;
        if (playerMoveFlg) {
        ...
        for(int i=0; i<ITEMNUM; i++){
            double dist = sqrt(pow(playerPos[0]-itemX[i],2)
                               +pow(playerPos[1]-itemY[i],2)
                               +pow(playerPos[2]-itemZ[i],2));
            ...
            if (dist<90.0 && playerPos[2]<itemZ[i] && itemStatus[i]==1){
                ...
                if(itemType[i]==0) {
                    ...
                    efctSize = 50.0;
                    efctAlpha = 1.5;
                    efctX = itemX[i]; efctY = itemY[i]; efctZ = itemZ[i]+30.0;
...
//                    photoImage = captureImage.clone();
        ...
        //efct
        efctSize *= 1.2;
        if (efctSize>700) efctSize = 700;
        efctAlpha *= 0.7;
    
    if(itemStatus[pachaItemID]==0){  //トロフィーを取っていたら
        pachaCnt--;
        if(pachaCnt==0){
            cv::Mat captureImage;
            capture >> captureImage;
            ...
            cdSize = 50.0;
            cdAlpha = 2.0;
            cdX = playerPos[0]; cdY = playerPos[1]; cdZ = playerPos[2]+200.0;
        }
        if(pachaCnt<0)pachaCnt = 0;
    }
...
void initGL()
{
    ...
    for (int i=0; i<2; i++) {
        ...
        col[0] = 5.0; col[1] = 5.0; col[2] = 5.0; col[3] = 1.0;
...
void analyzeBuffer()
{
    ...
    for (int i=0; i<bufferPoint; i++) {
        ...
        if (inData[dataID][j]==',') {  //カンマ（数値データの区切り）のとき
            inData[dataID][j] = '\0';  //文字列inData[k]完成
            dataID++;  //次の数値データに移動
